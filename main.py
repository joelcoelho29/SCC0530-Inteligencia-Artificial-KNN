...
def get_metrics(bfs_metrics, dfs_metrics, bf_metrics, astar_metrics):
    print("Metricas: \n")
    table_weight = [["Buscas", "BFS Weight", "DFS Weight", "BF Weight", "A* Weight"]]
    table_time = [["Buscas", "BFS Time", "DFS Time", "BF Time", "A* Time"]]
    table_visiteds = [["Buscas", "BFS Visiteds", "DFS Visiteds", "BF Visiteds", "A* Visiteds"]]

    for i in range(DISTINCT_PAIRS_TEST_AMOUNT-1):
        weight_line = [f"Busca {i+1}", bfs_metrics["weights"][i], dfs_metrics["weights"]
                       [i], bf_metrics["weights"][i], astar_metrics["weights"][i]]
        time_line = [f"Busca {i+1}", bfs_metrics["times"][i], dfs_metrics["times"]
                     [i], bf_metrics["times"][i], astar_metrics["times"][i]]
        visited_line = [f"Busca {i+1}", bfs_metrics["visiteds"][i], dfs_metrics["visiteds"]
                     [i], bf_metrics["visiteds"][i], astar_metrics["visiteds"][i]]
    ...
    results_weight_line = ["Total", bfs_total_weight,
                           dfs_total_weight, bf_total_weight, astar_total_weight]
    
    avg_weight_line = ["Media total de peso", bfs_total_weight / DISTINCT_PAIRS_TEST_AMOUNT, dfs_total_weight /
                       DISTINCT_PAIRS_TEST_AMOUNT,  bf_total_weight / DISTINCT_PAIRS_TEST_AMOUNT, astar_total_weight / DISTINCT_PAIRS_TEST_AMOUNT]

    results_time_line = ["Total", bfs_total_time,
                         dfs_total_time, bf_total_time, astar_total_time]
    
    avg_time_line = ["Media total de tempo", bfs_total_time / DISTINCT_PAIRS_TEST_AMOUNT, dfs_total_time /
                     DISTINCT_PAIRS_TEST_AMOUNT, bf_total_time / DISTINCT_PAIRS_TEST_AMOUNT, astar_total_time / DISTINCT_PAIRS_TEST_AMOUNT]

    results_visiteds_line = ["Total", bfs_total_visiteds,
                         dfs_total_visiteds, bf_total_visiteds, astar_total_visiteds]
    
    avg_visiteds_line = ["Media total de vertices visitados", bfs_total_visiteds / DISTINCT_PAIRS_TEST_AMOUNT, dfs_total_visiteds /
                     DISTINCT_PAIRS_TEST_AMOUNT, bf_total_visiteds / DISTINCT_PAIRS_TEST_AMOUNT, astar_total_visiteds / DISTINCT_PAIRS_TEST_AMOUNT]
...
def execute(vertices_pairs):
    ...
    for i, (source, target) in enumerate(vertices_pairs):
        ...
        weight, time, visiteds = execute_search_method(graph.best_first, source, target)
        bf_metrics["weights"].append(weight)
        bf_metrics["times"].append(time)
        bf_metrics["visiteds"].append(visiteds)

        weight, time, visiteds = execute_search_method(graph.a_star, source, target)
    ...
    get_metrics(bfs_metrics, dfs_metrics, bf_metrics, astar_metrics)
