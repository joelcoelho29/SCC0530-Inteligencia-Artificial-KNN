...
def execute_search_method(method, source, target):
    ...
    return (search_result[1], runtime)
...
def execute(vertices_pairs):
    bfs_metrics = {"weights": [], "times": []}
    dfs_metrics = {"weights": [], "times": []}
    bf_metrics = {"weights": [], "times": []}
    astar_metrics = {"weights": [], "times": []}
    ...
    for i, (source, target) in enumerate(vertices_pairs):
        ...
        weight, time = execute_search_method(
        ...
        weight, time = execute_search_method(graph.best_first, source, target)
        bf_metrics["weights"].append(weight)
        bf_metrics["times"].append(time)

        weight, time = execute_search_method(graph.a_star, source, target)
